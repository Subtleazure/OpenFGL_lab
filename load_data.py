...
from torch_geometric.datasets import Planetoid, WebKB, Amazon, WikipediaNetwork, Actor,Coauthor
import scipy.io
...
def even_quantile_labels(vals, nclasses, verbose=True):
    ...
    label = -1 * np.ones(vals.shape[0], dtype=int)  # 或者使用 np.int32 或 np.int64
    interval_lst = []
    lower = -np.inf
...
def load_dataset(dataname, train_prop, valid_prop, test_prop, num_masks):
    ...
    if dataname in ['cora', 'citeseer', 'pubmed']:
        dataset = Planetoid(root='/data0/wgc_data/graph/', name=dataname)
        data = dataset[0]
        ...
        dataset = WebKB(root='/data0/wgc_data/graph/', name=dataname)
        data = dataset[0]
        ...
        preProcDs = WikipediaNetwork(root='/data0/wgc_data/graph/', name=dataname, geom_gcn_preprocess=False, transform=T.NormalizeFeatures())
        dataset = WikipediaNetwork(root='/data0/wgc_data/graph/', name=dataname, geom_gcn_preprocess=True, transform=T.NormalizeFeatures())
        data = dataset[0]
        ...
        dataset = WikipediaNetwork2(root='/data0/wgc_data/graph/', name=dataname, geom_gcn_preprocess=False)
        data = dataset[0]
        data.y = data.y.long()
        ...
        dataset = Amazon(root='/data0/wgc_data/graph/', name=dataname)
        data = dataset[0]
        splits_lst = [rand_train_test_idx(data.y, train_prop=train_prop, valid_prop=valid_prop,test_prop=test_prop)
                      for _ in range(num_masks)]
        data.train_mask, data.val_mask, data.test_mask = index_to_mask(splits_lst, data.num_nodes)
        ...
        dataset = Coauthor(root='/data0/wgc_data/graph/', name=dataname,transform=T.NormalizeFeatures())
        data = dataset[0]
        splits_lst = [rand_train_test_idx(data.y, train_prop=train_prop, valid_prop=valid_prop,test_prop=test_prop)
                      for _ in range(num_masks)]
        data.train_mask, data.val_mask, data.test_mask = index_to_mask(splits_lst, data.num_nodes)
        ...
        dataset = HeterophilousGraphDataset(root='/data0/wgc_data/graph/', name=dataname)
        data = dataset[0]
        # data.y = data.y.squeeze()
        splits_lst = [rand_train_test_idx(data.y, train_prop=train_prop, valid_prop=valid_prop, test_prop=test_prop)
                      for _ in range(num_masks)]
...
def load_deezer():
    ...
    deezer = scipy.io.loadmat('/data0/wgc_data/graph/deezer-europe.mat')
...
def load_genius():
    filename = 'genius'
    # dataset = NCDataset(filename)
    fulldata = scipy.io.loadmat(f'/data0/wgc_data/graph/genius.mat')
...
def load_fb100(filename):
    ...
    mat = scipy.io.loadmat('/data0/wgc_data/graph/facebook100/' + filename + '.mat')
    A = mat['A']
...
def load_twitch(sub_dataname):
    assert sub_dataname in ('DE', 'ENGB', 'ES', 'FR', 'PTBR', 'RU', 'TW'), 'Invalid dataset'
    filepath = '/data0/wgc_data/graph/twitch/' + sub_dataname
    label = []
    node_ids = []
    src = []
    targ = []
