...
def load_global_dataset(root, scenario, dataset):
    ...
    if scenario == "graph_fl":
        if dataset in ["AIDS",
            ...
            from torch_geometric.datasets import TUDataset
            tudataset = TUDataset(root=osp.join(
                root, "graph_fl"), name=dataset, use_node_attr=True, use_edge_attr=True)
            max_degree = 0
...
class hERGDataset(InMemoryDataset):
    ...
    def raw_file_names(self) -> List[str]:
        names = [
            'adjacency_matrices.pkl', 'edge_feature_matrices.pkl',
            'feature_matrices.pkl', 'labels.npy', 'smiles.pkl'
        ]
        return names
    ...
    def process(self) -> None:
        with open(osp.join(self.raw_dir, "adjacency_matrices.pkl"),
                  'rb') as file:
        ...
        batch = torch.hstack([
            torch.tensor([graph_i] * num_nodes_list[graph_i])
            for graph_i in range(num_graphs)
        ])

        with open(osp.join(self.raw_dir, "feature_matrices.pkl"),
                  'rb') as file:
            node_feature_list = pickle.load(file)
        node_attribute = torch.vstack([
            torch.tensor(node_feature_np)
            for node_feature_np in node_feature_list
        ])

        node_label = torch.empty((batch.size(0), 0))

        with open(osp.join(self.raw_dir, "edge_feature_matrices.pkl"),
                  'rb') as file:
            edge_feature_list = pickle.load(file)
        edge_attribute = torch.vstack([
            torch.tensor(edge_feature_np)[:, 2:]
            for edge_feature_np in edge_feature_list
        ])
...
class WikiPages(InMemoryDataset):
    ...
    def download(self) -> None:
        fs.cp(f"{self.url}/{self.name.lower()}.zip",
              self.raw_dir,
              extract=True)
        print(self.raw_dir)
        print(self.root)
    ...
    def process(self) -> None:
        ...
        node_feature_label_path = osp.join(self.raw_dir,
        ...
        with open(node_feature_label_path, 'r') as file:
            ...
            for line in file:
                ...
                node_feature_list.append(
                    [int(num) for num in feature.split(',')])
