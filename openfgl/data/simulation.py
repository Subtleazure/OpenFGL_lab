...
def graph_fl_cross_domain(args, global_dataset):
    ...
    for client_id in range(args.num_clients):
        # list(InMemoryDataset) -> InMemoryDataset
...
def graph_fl_label_skew(args, global_dataset, shuffle=True):
    ...
    while min_size < args.least_samples:
        ...
        for k in range(K):
            ...
            client_indices = [idx_j + idx.tolist() for idx_j,
                              idx in zip(client_indices, np.split(idx_k, proportions))]
    ...
    for client_id in range(args.num_clients):
        for class_i in range(K):
            client_label_counts[client_id][class_i] = (
                graph_labels[client_indices[client_id]] == class_i).sum()
        ...
        # InMemoryDataset -> deep-copy subset
        local_graphs = global_dataset.copy(client_indices[client_id])
...
def subgraph_fl_label_skew(args, global_dataset, shuffle=True):
    ...
    for client_id in range(args.num_clients):
        for class_i in range(K):
            client_label_counts[client_id][class_i] = (
                node_labels[client_indices[client_id]] == class_i).sum()
        local_subgraph = get_subgraph_pyg_data(
            global_dataset, client_indices[client_id])
...
def subgraph_fl_louvain_plus(args, global_dataset):
    ...
    for node_id, com_id in enumerate(fit_result):
        if com_id not in communities:
            communities[com_id] = {"nodes": [], "num_nodes": 0, "label_distribution": [
                0] * global_dataset.num_classes}
    ...
    clustering_data = np.zeros(
        shape=(num_communities, global_dataset.num_classes))
    for com_id in communities.keys():
    ...
    for com_id in range(num_communities):
        client_indices[clustering_labels[com_id]
                       ] += communities[com_id]["nodes"]
...
def subgraph_fl_metis(args, global_dataset):
    ...
    for client_id in range(args.num_clients):
        client_indices[client_id] = np.where(
            np.array(membership) == client_id)[0].tolist()
