...
supported_graph_fl_simulations = [
    "graph_fl_cross_domain", "graph_fl_label_skew", "graph_fl_topology_skew", "graph_fl_feature_skew"]
supported_subgraph_fl_simulations = ["subgraph_fl_label_skew", "subgraph_fl_louvain_plus",
                                     "subgraph_fl_metis_plus", "subgraph_fl_louvain", "subgraph_fl_metis"]

supported_graph_fl_task = ["graph_cls", "graph_reg"]
supported_subgraph_fl_task = ["node_cls", "link_pred", "node_clust"]


supported_fl_algorithm = ["isolate", "fedavg", "fedprox", "scaffold", "moon", "feddc", "fedproto", "fedtgp",
                          "fedpub", "fedstar", "fedgta", "fedtad", "gcfl_plus", "fedsage_plus", "adafgl", "feddep", "fggp", "fgssl", "fedgl"]


supported_metrics = ["accuracy", "precision", "f1",
                     "recall", "auc", "ap", "clustering_accuracy", "nmi", "ari"]


supported_evaluation_modes = ["global_model_on_local_data", "global_model_on_global_data",
                              "local_model_on_local_data", "local_model_on_global_data"]
...
parser.add_argument("--scenario", type=str,
                    default="subgraph_fl", choices=supported_scenario)
parser.add_argument("--dataset", type=str, default=[], action='append')
parser.add_argument("--processing", type=str, default="raw",
                    choices=supported_data_processing)
...
parser.add_argument("--fl_algorithm", type=str,
                    default="fedavg", choices=supported_fl_algorithm)
parser.add_argument("--client_frac", type=float, default=1.0)


# simulation settings
parser.add_argument("--simulation_mode", type=str, default="subgraph_fl_louvain",
                    choices=supported_graph_fl_simulations + supported_subgraph_fl_simulations)
...
parser.add_argument("--task", type=str, default="node_cls",
                    choices=supported_graph_fl_task + supported_subgraph_fl_task)
parser.add_argument("--num_clusters", type=int, default=7)
# training settings
parser.add_argument("--train_val_test", type=str,
                    default="default_split")  # e.g., 0.2-0.4-0.4
...
parser.add_argument("--evaluation_mode", type=str,
                    default="local_model_on_local_data", choices=supported_evaluation_modes)
...
parser.add_argument("--accuracy_curve_dir", type=str, default=None)
parser.add_argument("--accuracy_curve_html_dir", type=str, default=None)
...
parser.add_argument("--aggregation_mode", type=str, default="ben", choices=supported_aggregation_modes)
parser.add_argument("--dataset_", type=str, default=None)
parser.add_argument("--graph_repair", type=bool, default=False)
parser.add_argument("--S_ano_list", type=list, default=[])
parser.add_argument("--repair_clients", type=list, default=[])
parser.add_argument("--contamination_ratio", type=float, default=0.3)
parser.add_argument("--heatmap", type=bool, default=False)
parser.add_argument("--contaminated_client_indices", type=list, default=[])

parser.add_argument("--rhfl", type=bool, default=True)
parser.add_argument("--current_mean_loss_list", type=list, default=[])
parser.add_argument("--last_mean_loss_list", type=list, default=[])
